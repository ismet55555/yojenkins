...
class Auth:
    ...
    def generate_token(self, token_name:str='', server_base_url:str='', username:str='', password:str='') -> str:
        ...
        request_return_text, _, success = self.R.request(
        ...
        request_return_content, _, success = self.R.request(
            target=server_base_url.strip('/') + '/me/descriptorByName/jenkins.security.ApiTokenProperty/generateNewToken/api/json',
            is_endpoint=False,
            request_type='post',
            json_content=True,
            auth=(username, password),
    ...
    def profile_add_new_token(self, profile_name:str) -> str:
        ...
        logger.debug(f'Currently listed profile names: {", ".join(list(profiles[self._required_top_level_key].keys()))}')
    ...
    def configure(self, api_token:str='') -> bool:
        ...
        if file_exists:
            ...
            if self._required_top_level_key not in profiles.keys():
                logger.debug(f'The loaded credentials configuration file does not include a "{self._required_top_level_key}" section')
                return False
            logger.debug(f'Currently listed profile names: {", ".join(list(profiles[self._required_top_level_key].keys()))}')
            ...
            print(f'Creating a new credentials profile file: {os.path.join(file_path, self._cred_config_filename)} ...')
    ...
    def detect_configuration_file(self) -> Tuple[bool, str]:
        ...
        if self._cred_config_filename  in files_in_home_dir:
            ...
            logger.debug(f'Configuration file "{self._cred_config_filename }" found in user home directory: {Path.home()}')
        ...
        if not profile_selected:
            if self._profile_env_var in os.environ:
                ...
                if profile in profile_items:
                    profile_selected = profile_items[profile]
                    profile_selected['profile'] = profile
                    logger.debug(f'Successfully matched set {self._profile_env_var } environmental variable value "{profile}"')
                else:
                    logger.debug(f'Failed to find the set environmental variable {self._profile_env_var} "{profile}" in the profiles loaded')
                    return {}
            else:
    ...
    def create_auth(self, profile_info:dict={}) -> bool:
        ...
        if not url_protocol_schema:
            logger.debug(f'Failed to find a valid server URL protocol schema (ie. http://, https://, etc) in the loaded profile server url: "{self.jenkins_profile["jenkins_server_url"]}"')
            return False
    ...
    def verify(self) -> bool:
        ...
        try:
            ...
            return False
        request_success = self.R.request(
            target=request_url,
            is_endpoint=False,
            request_type='head'
            )[2]
